import pandas as pd
data = pd.read_csv("https://raw.githubusercontent.com/ldkrsi/cpbl-opendata/master/CPBL/standings.csv") #載入資料
data = data.drop(columns=['Old ID','Team Nickname','Tie'])#除去不需要的資料
#初始化年分
start=0
end=0

def inquiry(start,end):
...
while True:
    try:
        ...
        if (start <= end and start >= 1990 and end <= 2022):
            print(inquiry(start,end))
